...
public class GraphApplication {
    ...
    public static void main(String[] args) {
        ...
        ArrayList<Graph<String, Double>> prim2 = new ArrayList<>();
        int i = 0;
        while(prim.numNodes() + i < graph.numNodes()){
            for(String v : graph.getNodes()){
                if(!prim.containsNode(v)){
                    boolean val = false;
                    for(Graph<String, Double> g : prim2){
                        if(!g.containsNode(v)){
                            val = true;
                            break;
                        }
                    ...
                    if(!val){
                        prim2.add(minimumSpanningTreePrim(graph, v));
                    }
                }
            }
        ...
        System.out.println("Graph's size after Prim execution: " + total);
        ...
        for(Graph<String, Double> g : prim2){
            for(AbstractEdge<String, Double> e : g.getEdges()){
                sum += e.getLabel();
            }
        }
